...
const AudioFingerprintRuntime = () => {
  ...
  function ht(t, e) {
    ...
    try { Object.defineProperty(fn, 'name', { value: t }) } catch (_) {}
    return fn
  }
  ...
  function le(t, e, r, n, o) {
    ...
    return (function () {
      ...
      var hasReturn = c
      var hasThis = a
      var useDestructorsArray = u
      var wrapper = function () {
        if (arguments.length !== numArgs) {
          vt('function ' + t + ' called with ' + arguments.length + ' arguments, expected ' + numArgs + ' args!')
        }
        var destructors = useDestructorsArray ? [] : null
        var d = useDestructorsArray ? destructors : null
        var thisWired = null
        if (hasThis) {
          thisWired = e[1].toWireType(d, this)
        }
        var wiredArgs = []
        for (var s = 0; s < numArgs; ++s) {
          wiredArgs.push(e[s + 2].toWireType(d, arguments[s]))
        }
        var invokerArgs = [o]
        if (hasThis) invokerArgs.push(thisWired)
        for (var s = 0; s < wiredArgs.length; ++s) {
          invokerArgs.push(wiredArgs[s])
        }
        var rv = n.apply(null, invokerArgs)
        if (useDestructorsArray) {
          fe(destructors)
        } else {
          for (var s = hasThis ? 1 : 2; s < e.length; ++s) {
            if (e[s] !== null && e[s].destructorFunction) {
              if (s === 1) {
                e[s].destructorFunction(thisWired)
              } else {
                e[s].destructorFunction(wiredArgs[s - 2])
              }
            }
          }
        }
        if (hasReturn) {
          return e[0].fromWireType(rv)
        }
      }
      try { Object.defineProperty(wrapper, 'name', { value: dt(t) }) } catch (_) {}
      return wrapper
    })()
  }
  const pe = [],
  ...
  var Se,
    ...
    ke =
      ((function () {
        ...
        if ((J++, o.monitorRunDependencies && o.monitorRunDependencies(J), o.instantiateWasm = function(imports, cb) {
          var instance = new WebAssembly.Instance(_wasmModule, imports);
          cb(instance);
          return {};
        }))
          try {
...
// 缓存 WASM runtime，避免每次生成指纹都重新实例化
let _cachedRuntime = null
let _runtimeInitPromise = null

async function getRuntime() {
  if (_cachedRuntime && typeof _cachedRuntime.ExtractQueryFP === 'function') {
    return _cachedRuntime
  }
  if (_runtimeInitPromise) {
    return _runtimeInitPromise
  }
  _runtimeInitPromise = instantiateRuntime().then(rt => {
    _cachedRuntime = rt
...
function GenerateFP(floatArray) {
  ...
  return getRuntime().then((fpRuntime) => {
    ...
    // 如果缓存的 runtime 失败，清除缓存并重试一次
    console.error('[afp] fingerprinting failed:', err.message)
    _cachedRuntime = null
    _runtimeInitPromise = null
    throw err
  })
